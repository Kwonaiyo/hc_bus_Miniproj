...
def result():
    ...
    # 여기까지 1초~3초
    arsno = [] # 노선id들을 바탕으로 각 버스의 현재 입력된 정류장id와 다음정류장 이름을 가져옴         정류장 이름으로 두개의 id가 나왔는데
    ...
    #return render_template("bus_sample.html", lineid=result_dict)#, time = end-start)

@app.route("/final_result", methods = ["POST"])
def final_result():
    value = request.form.get('value')  # value : 클릭한 버튼(다음정류장)의 arsno
    name = request.form.get('name')    # 클릭한 버튼의 name?
    temp_list = []
